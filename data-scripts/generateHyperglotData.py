...
def contains_lowercase(cp, code_points):
    ...
    try:
        return (
            uniLowerCaseMapping[cp] in code_points
            and uniUpperCaseMapping[uniLowerCaseMapping[cp]] == cp
        )
    except KeyError:
        return False
...
for code, data in languages:
    ...
    for orthography in orthographies:
        ...
        if script in language_dict:
            ...
            if "auxiliary" in orthography:
                unicodes_dict["optional"] = code_points_reduced(
                    orthography["auxiliary"]
                )
            name = data["name"]
            if script != "DFLT":
